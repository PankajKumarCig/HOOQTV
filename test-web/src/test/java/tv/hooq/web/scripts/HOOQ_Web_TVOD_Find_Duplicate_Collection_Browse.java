...
public class HOOQ_Web_TVOD_Find_Duplicate_Collection_Browse extends HOOQWorkflows {
	/*
...
ExcelReader xlsLogin = new ExcelReader(configProps.getProperty("WEB_TEST_DATA"),"Subs_04");
	...
	public  void HOOQ_Web_TVOD_Find_Duplicate_Collection_Browse (String email,String type,String userType,String description) throws Throwable {
		try{
			...
			//Get the Collection Name
				String[] strCollection=fnGetBrowseMovieCollectionName();
				for(int i=0;i<strCollection.length;i++)
				{
					Reporter.reportStep("Verifying Duplicate Collection On Browse Section " +strCollection[i] );
					System.out.println(strCollection[i]);
					boolean blnCollection=fnClickBrowseCollection(strCollection[i]);
					System.out.println(strCollection[i] + " ==> " + blnCollection);
					String[] strCollectionTitle=fnGetCollectionList(strCollection[i],"");
